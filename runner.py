...
import sys
import os
from terminal_display import parse_schedule, print_weekly_schedule, print_shift_statistics, print_optimization_cost

# Configure logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

def run_dlv(base_file, opt_file, live=False, time_limit=None):
    logging.info(f"Running DLV solver with files: {base_file}, {opt_file}")
    if live:
        logging.warning("Live printing is currently fully supported only with --clingo. DLV will process normally.")
        
    try:
        process = subprocess.Popen(
            ['dlv', base_file, opt_file],
        ...
        if process.returncode != 0 and process.returncode is not None:
            logging.error(f"DLV execution failed: {stderr}")
            sys.exit(1)
        return stdout
    except subprocess.TimeoutExpired:
        logging.warning(f"Time limit of {time_limit}s reached. Terminating DLV...")
        process.terminate()
        stdout, _ = process.communicate()
        return stdout
    except KeyboardInterrupt:
        logging.warning("Execution interrupted by user (Ctrl+C). Terminating DLV...")
        process.terminate()
        stdout, _ = process.communicate()
        return stdout
    except FileNotFoundError:
        logging.error("DLV executable not found. Please ensure it is installed and in your PATH.")
        sys.exit(1)
...
def main():
    ...
    parser.add_argument('--opt', required=True, choices=['differenza_turni', 'differenza_turni_con_penalita', 'penalita_esponenziale'],
    ...
    solver_group.add_argument('--dlv', action='store_true', default=True, help="Use DLV solver (default).")
    solver_group.add_argument('--clingo', action='store_true', help="Use Clingo solver via Python API.")

    args = parser.parse_args()

    # If --clingo is explicitly passed, disable the default --dlv
    if args.clingo:
        args.dlv = False

    base_file = "base.asp"
    opt_file = os.path.join("optimizations", f"{args.opt}.asp")
    ...
    if args.clingo:
        ...
        asp_output = run_dlv(base_file, opt_file, live=args.live, time_limit=args.time_limit)
